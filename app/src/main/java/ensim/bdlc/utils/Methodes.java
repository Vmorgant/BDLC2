...
import ensim.bdlc.Activities.ActivityAccueil;
import ensim.bdlc.Activities.ActivityStart;
import ensim.bdlc.R;
...
public class Methodes {
    ...
    public static String date(){
        String string_jour_system,string_mois_system;
        int jour_sytem,mois_system;

        Calendar c = Calendar.getInstance();

        /**jour*/
        jour_sytem = c.get(Calendar.DATE);
        if( jour_sytem<10){
            string_jour_system = "0"+jour_sytem;
        }else{
            string_jour_system = ""+jour_sytem;
        }
        /**mois*/
        mois_system = c.get(Calendar.MONTH);
        mois_system++;
        if( mois_system<10){
            string_mois_system = "0"+mois_system;
        }else{
            string_mois_system = ""+mois_system;
        }

        return ""+string_jour_system+"/"+string_mois_system+"/"+c.get(Calendar.YEAR);
    }
}
